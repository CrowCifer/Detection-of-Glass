...
void create_contours() //提取轮廓
{
	...
	blur(src_gray, src_gray, Size(3, 3));        //对图像进行滤波
...
bool isStore(int minC, int minR, int maxC, int  maxR) //判断连通域是否保留
{
	int CL = maxC - minC, RL = maxR - minR;
	double rate = CL * 1.0 / RL;
	if (fabs(rate - 1) <= eps && RL >= 20 && CL >= 20) //判断条件
	{
		return true;
	}
	return false;
...
void find_adjacent() //提取连通域
{
	...
	for (int r = lr; r < rr; r++)
	{
		...
		{
			tmpPoint.clear();
			if (!used[r][c] && dst.at<uchar>(r, c) == 255) //发现一个点值为255
			{
